...
const youtube = new YouTube('AIzaSyCjecMoZu1pWrXEavIf8iZofqt3qPv-Cag');
const queue = new Map();
...
client.on('message', msg => {
  if (msg.content === 'r!kayıt')	{
   msg.delete(1)
  }
});

client.on('message', msg => {
  if (msg.content === 'r!kayit')	{
   msg.delete(1)
  }
});
...
client.on('ready', ()=>{
client.channels.get('695638219697684610').startTyping()
})
...
const yourID = "614197519865348096"; //Instructions on how to get this: https://redd.it/40zgse //Kendi İD'nizi Yazın
const setupCMD = "r!duyurual" //İstediğiniz Komut Yapabilirsiniz örn : !kayıtol
let initialMessage = `Sıkı takipçi`; //Dilediğiniz Şeyi Yazabilirsiniz
const roles = ["Sıkı Takipçiler"]; //İstediğiniz Rolü Yazabilirsiniz
const reactions = ["❌"]; //İstediğiniz Emojiyi Ekleyebilirsiniz
const botToken = "Njk4Nzc3Njc3NjQ0OTU1NzMw.XrhOAA.k9bA6-nUfFSISKVcA_wbQDC3x1Y";  //Buraya botunuzun tokenini koyunuz
...
function generateMessages(){
    ...
    for (let role of roles) messages.push(`**"${roles}"** rolünü almak için ❌ Emojisine Tıkla!`); //DONT CHANGE THIS
    return messages;
}
...
client.on("message", async msg => {
  const db = require('quick.db');
   if (msg.content === 'r!oyla') {
    msg.reply('Merhabalar. Beni oylayarak ekibime destek verebilirsiniz. :heart: https://top.gg/bot/698777677644955730/vote'); //${client.guilds.size}
  }
